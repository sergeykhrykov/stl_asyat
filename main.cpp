...
int main(int argc, char* argv[])
{
	...
	else // Файл бинарный 
	{
		...
		for (unsigned int i = 0; i < 10; i++) //30треугольников 
											  //for (unsigned int i = 0; i < Number_of_triangles; i++) //считывает все треугольники 
		{
			fin.read((char*)s.Normal_vector, sizeof(s.Normal_vector));

			for (int i = 0; i < 3;
			17:05 : 03
				i++) // Считываем вершины треугольника 
			{
	...
	else { cout « "Обнаружены ошибки геометрии." « endl; _getch(); return 1; }
	...
	vector<Point> pts;
	for (auto pt : points) pts.push_back(pt.first);
